--- 1. Príprava dát: Vytvorenie databázy a schémy
...
--- 3. Transform: Čistenie a príprava dát (Staging vrstva)
-- Zjednotenie historických dát a predpovedí, ošetrenie chýbajúcich hodnôt (COALESCE)
...
-- Čistenie dát pre klimatologické normy
...
--- 4. Load: Tvorba dimenzionálneho modelu (DWH vrstva)
CREATE OR REPLACE SCHEMA GlobalWeather_DB.DWH;
USE SCHEMA GlobalWeather_DB.DWH;

-- dim_date: Časová dimenzia s atribútmi sezón a víkendov
...
-- dim_location: Geografická dimenzia s deduplikáciou lokalít
...
-- dim_weather_type: Rozlíšenie medzi HISTORICAL a FORECAST dátami
...
--- 5. Tvorba faktových tabuliek (Galaxy Schema)
-- fact_weather_measurements: Hlavná faktová tabuľka s využitím Window Functions
...
--- 6. Odstránenie staging tabuliek (Cleanup)
